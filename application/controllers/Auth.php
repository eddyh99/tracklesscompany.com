...
class Auth extends CI_Controller
{
	...
	public function mailproses()
	{
		...
		$this->form_validation->set_rules('vemail', 'vemail', 'trim|required');
		$this->form_validation->set_rules('companyt', 'companyt', 'trim|required');
		$this->form_validation->set_rules('usphone0', 'usphone0', 'trim');
		$this->form_validation->set_rules('usphone1', 'usphone1', 'trim');
		$this->form_validation->set_rules('usphone2', 'usphone2', 'trim');
		$this->form_validation->set_rules('usphone3', 'usphone3', 'trim');
		$this->form_validation->set_rules('interphone', 'interphone', 'trim');
		$this->form_validation->set_rules('company', 'company', 'trim|required');
		$this->form_validation->set_rules('streetadd1', 'streetadd1', 'trim|required');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			redirect(base_url("auth/delselect"));
			return;
		}

		$input = $this->input;
		$fname = $this->security->xss_clean($input->post("fname"));
		$email = $this->security->xss_clean($input->post("email"));
		$phonetype = $this->security->xss_clean($input->post("phonetype"));
		$usphone0 = $this->security->xss_clean($input->post("usphone0"));
		$usphone1 = $this->security->xss_clean($input->post("usphone1"));
		$usphone2 = $this->security->xss_clean($input->post("usphone2"));
		$usphone3 = $this->security->xss_clean($input->post("usphone3"));
		$interphone = $this->security->xss_clean($input->post("interphone"));
		$company = $this->security->xss_clean($input->post("company"));
		$streetadd1 = $this->security->xss_clean($input->post("streetadd1"));
		$streetadd2 = $this->security->xss_clean($input->post("streetadd2"));
		$city = $this->security->xss_clean($input->post("city"));
		$country = $this->security->xss_clean($input->post("country"));

		if ($phonetype == 'us') {
			$phonenumber = $usphone0 . $usphone1 . $usphone2 . $usphone3;
		} else {
			$phonenumber = $interphone;
		}

		if ($streetadd2 == '') {
			$streetadd = $streetadd1;
		} else {
			$streetadd = $streetadd1 . " (" . $streetadd2 . ")";
		}
		...
		$message = "
			...
			Company : " . $company . "<br>
			Street address : " . @$streetadd . "<br>
			City : " . $city . "<br>
			Country : " . $country;

		$result = sendmail($this->phpmailer_lib->load(), $email, $message);
		$this->session->set_flashdata("success", "Message successfully sent!");
		redirect('auth/delselect');
	}
}