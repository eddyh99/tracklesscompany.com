<nav class="navbar bg-black">
    <div class="container-fluid">
        <a class="navbar-brand" href="<?= base_url(); ?>">
            <img src="<?= base_url(); ?>assets/images/tracklesscompany-fit.png">
        </a>
        <a href="https://tracklessproject.com" class="bg-trackless me-5" data-bs-toggle="offcanvas"
            data-bs-target="#offcanvasNavbar" aria-controls="offcanvasNavbar">
            <svg width="57" height="53" viewBox="0 0 57 53" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" clip-rule="evenodd"
                    d="M26.908 1.37204C27.8784 0.750986 29.1214 0.750986 30.0918 1.37204L54.7011 17.122C56.0749 18.0012 56.4758 19.8276 55.5966 21.2013C54.7174 22.575 52.8911 22.9759 51.5174 22.0967L49.1717 20.5955V44.2188C49.1717 48.568 45.646 52.0938 41.2967 52.0938H15.703C11.3538 52.0938 7.828 48.568 7.828 44.2188V20.5955L5.48239 22.0967C4.10868 22.9759 2.28235 22.575 1.40317 21.2013C0.523991 19.8276 0.924893 18.0012 2.29861 17.122L26.908 1.37204ZM19.1483 24.0391C18.3328 24.0391 17.6717 24.7001 17.6717 25.5156C17.6717 26.3311 18.3328 26.9922 19.1483 26.9922H37.8514C38.6669 26.9922 39.328 26.3311 39.328 25.5156C39.328 24.7001 38.6669 24.0391 37.8514 24.0391H19.1483ZM17.6717 31.4219C17.6717 30.6064 18.3328 29.9453 19.1483 29.9453H27.0233C27.8388 29.9453 28.4999 30.6064 28.4999 31.4219C28.4999 32.2374 27.8388 32.8984 27.0233 32.8984H19.1483C18.3328 32.8984 17.6717 32.2374 17.6717 31.4219ZM19.1483 35.8516C18.3328 35.8516 17.6717 36.5126 17.6717 37.3281C17.6717 38.1436 18.3328 38.8047 19.1483 38.8047H30.9608C31.7763 38.8047 32.4374 38.1436 32.4374 37.3281C32.4374 36.5126 31.7763 35.8516 30.9608 35.8516H19.1483Z"
                    fill="#00DD9C" />
            </svg>
        </a>
    </div>
</nav>
<!-- Mashead header-->
<header class="slider slider-tracklesscompany-3 py-5">
    <div class="container-fluid px-5">
        <div class="row d-flex align-items-center justify-items-center min-hight company-indonesia">
            <div class="col-lg-12">
                <!-- Mashead text and app badges-->
                <div class="mb-5 mb-lg-0 text-center">
                    <h1 class="text-slider-indo mb-3 f-poppins">WELCOME TO DELAWARE</h1>
                </div>
                <div class="row d-flex justify-content-center align-items-center flex-column">
                    <div class="col-12 col-sm-6 col-lg-4 my-3">
                        <a href="" class="btn-indo f-inter py-3 px-5 text-center">
                            DELAWARE<br>
                            <i>Offshore company</i>
                        </a>
                        <p class="f-poppins text-start mt-2">High level of corporate and monetary privacy. FLAT TAX </p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</header>

<section class="py-5">
    <div class="container-fluid px-md-5">
        <div class="row gx-5 align-items-center">
            <div class="col-12">
                <div class="indonesia-country text-center">
                    <h1 class="my-5 f-poppins">
                        <div class="line-r rounded-start border-end-0 d-none d-lg-grid"></div>
                        <div class="line-l rounded-end border-start-0 d-none d-lg-grid"></div>
                        What makes Delaware a great place for company formation ?
                    </h1>
                </div>
                <div class="text-center content-indonesia">
                    <h2 class="f-poppins">Delaware</h2>
                    <p class="f-inter my-5">Delaware, a State of the United States of America, is considered a kind of
                        free port. Over the years it has become one of the most accredited places to establish company
                        headquarters. Delaware companies enjoy absolutely favorable legislation that protects capital,
                        corporate earnings with flat taxation, and the privacy of share holders and directors. The names
                        of the latter are classified and are not communicated even to the Public Offices of the State of
                        Delaware.
                        It is also a White List State and being part of the United States it will never go black or gray
                        list</p>
                </div>
                <div class="row d-flex justify-content-center">
                    <div class="col-12 col-sm-8 col-lg-4">
                        <div class="p-2">
                            <div
                                class="box-price h-auto px-4 mb-5 py-5 f-poppins d-flex align-items-center flex-column mb-3">
                                <div class="type-price easy py-3 px-5">
                                    Easy
                                </div>
                                <div class="col-12 box-list my-3">
                                    <p>The most economical and environmentally-friendly way to form an LLC or
                                        corporation</p>
                                    <div class="d-flex align-items-center flex-row">
                                        <span class="list-title me-3">Include</span>
                                        <span class="list-price">$ 275</span>
                                    </div>
                                    <ul>
                                        <li>All Delaware Fees</li>
                                        <li>Registered Agent Fee</li>
                                        <li>Name Check & Clearance</li>
                                        <li>Preparation of Documents</li>
                                        <li>Same-day Electronic Filing</li>
                                        <li>Certificate of Formation/Incorporation</li>
                                        <li>Digital Copy of Documents</li>
                                        <li>Digital Corporate Seal</li>
                                        <li>One Complete Year (12 Full Months) of
                                            Delaware Registered Agent Service</li>
                                    </ul>
                                    <ul>
                                        <li>Lowest Delaware Registered Agent Fee</li>
                                        <li>Lifetime Customer Support</li>
                                        <li>FREE Compliance Coaching</li>
                                    </ul>
                                    <span class="list-title md">Additional</span>
                                    <div class="d-flex">
                                        <div class="p-2 w-50">Annual flat tax </div>
                                        <div class="p-2 flex-shrink-1">: $ 500</div>
                                    </div>
                                    <div class="d-flex">
                                        <div class="p-2 w-50">Agent fee </div>
                                        <div class=" p-2 flex-shrink-1">: $ 100</div>
                                    </div>
                                </div>
                                <a class="btn-order py-3 px-5 mt-5" href="<?= base_url() ?>auth/delselect">SELECT</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- Footer-->
<footer class="bg-footer-trackless py-3">
    <div class="container-fluid px-md-5">
        <div class="container">
            <div class="bg-imagebox">
                <img src="<?= base_url() ?>assets/images/trackless-fit.png" alt="" class="footer-logo-center">
            </div>
            <div class="d-flex justify-content-around">
                <div class="col-4">
                    <img src="<?= base_url(); ?>assets/images/footer-logo.png" class="footer-logo">
                    <div class="d-flex flex-wrap icon-sosmed">
                        <div class="me-md-2 me-lg-2 me-xl-3 mt-1 mt-sm-3">
                            <img src="<?= base_url() ?>assets/images/fb-icon.png" class="p-1">
                        </div>
                        <div class="me-md-2 me-lg-2 me-xl-3 mt-1 mt-sm-3">
                            <img src="<?= base_url() ?>assets/images/twitter-icon.png" class="p-1">
                        </div>
                        <div class="me-md-2 me-lg-2 me-xl-3 mt-1 mt-sm-3">
                            <img src="<?= base_url() ?>assets/images/ig-icon.png" class="p-1">
                        </div>
                        <div class="me-md-2 me-lg-2 me-xl-3 mt-1 mt-sm-3">
                            <img src="<?= base_url() ?>assets/images/c-icon.png" class="p-1">
                        </div>
                        <div class="me-md-2 me-lg-2 me-xl-3 mt-1 mt-sm-3">
                            <img src="<?= base_url() ?>assets/images/linkedin-icon.png" class="p-1">
                        </div>
                        <div class="me-md-2 me-lg-2 me-xl-3 mt-1 mt-sm-3">
                            <img src="<?= base_url() ?>assets/images/tiktok-icon.png" class="p-1">
                        </div>
                        <div class="me-md-2 me-lg-2 me-xl-3 mt-1 mt-sm-3">
                            <img src="<?= base_url() ?>assets/images/youtube-icon.png" class="p-1">
                        </div>
                    </div>
                </div>
                <div class="col-4">&nbsp;</div>
                <div class="col-4 footer-contact">
                    <h2>Contact us</h2>
                    <span>Telegramchanel</span>
                </div>
            </div>
        </div>
        <div class="d-flex flex-wrap justify-content-end footer-menus" translate="no">
            <a href="https://tracklessproject.com" class="">TracklessProject</a> |
            <a href="https://tracklessmail.com" class="">TracklessMail</a> |
            <a href="https://tracklesschat.com" class="">TracklessChat</a> |
            <a href="https://tracklesscompany.com" class="active">TracklessCompany</a> |
            <a href="https://tracklessbank.com" class="">TracklessBank</a> |
            <a href="https://tracklesscrypto.com" class="">TracklessCrypto</a> |
            <a href="https://tracklessmoney.com" class="">TracklessMoney</a>
        </div>
    </div>
</footer>